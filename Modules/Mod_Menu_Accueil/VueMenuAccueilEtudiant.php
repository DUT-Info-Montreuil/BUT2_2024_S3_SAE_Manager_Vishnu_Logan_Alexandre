...
class VueMenuAccueil extends VueGenerique {
    ...
    public function afficherAccueil() {
        ...
        foreach ($data as $annee) {
            ...
            echo "<button class='add-semester'><a href='index.php?module=mod&action=ajouterSemestre&annee_id=" . $annee['id'] . "'>Ajouter un semestre</a></button>";
            ...
            foreach ($annee['semestres'] as $semestre) {
                ...
                foreach ($semestre['projets'] as $projet) {
                    ...
                    echo "<button class='delete'><a href='index.php?module=mod&action=supprimerProjet&id=$projet_id'>Supprimer</a></button>";                    echo "</div>";
                }
                echo "<button class='delete-semester'><a href='index.php?module=mod&action=supprimerSemestre&id=" . $semestre['id'] . "'>Supprimer le semestre</a></button>";

                echo "<button class='add-project'><a href='index.php?module=mod&action=ajouterProjet&semestre_id=" . $semestre['id'] . "'>Ajouter un projet</a></button>";
                echo "</div>";
                echo "</div>"; 
            }
    ...
    public function afficherFormulaireAjoutProjet($semestre_id) {
        ...
        echo "<form method='POST' action='index.php?module=mod&action=enregistrerProjet'>";
    ...
    public function afficherFormulaireAjoutSemestre($annee_id) {
        ...
        echo "<form method='POST' action='index.php?module=mod&action=enregistrerSemestre'>";
